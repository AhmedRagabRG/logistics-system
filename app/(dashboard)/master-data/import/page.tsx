...
> = {
  ...
  route_pricing: {
    sheetNote:
      'First sheet only. The first 2 rows are treated as headers — actual data starts from row 3. Columns: B=origin, C=destination, D=is_sea_active, E=road_price, F=currency, G=sea_price, H=sea_currency, I=reverse_road_price, J=reverse_sea_price.',
    columns: [
      { key: 'B — Origin Region', label: 'Origin Region', required: true },
      { key: 'C — Destination Region', label: 'Destination Region', required: true },
      { key: 'D — Sea Active', label: 'Sea Active (Yes/No)', required: false },
      { key: 'E — Road Price', label: 'Road Price', required: true },
      { key: 'F — Currency', label: 'Currency', required: false },
      { key: 'G — Sea Price', label: 'Sea Price', required: false },
      { key: 'H — Sea Currency', label: 'Sea Currency', required: false },
      { key: 'I — Reverse Road Price', label: 'Reverse Road Price', required: false },
      { key: 'J — Reverse Sea Price', label: 'Reverse Sea Price', required: false },
    ],
    example: {
  ...
  vendors: {
    ...
    columns: [
      ...
      },
      {
        ...
        label: 'Email',
        required: false,
        alternatives: ['MAIL İHRACAT', 'email', 'E-mail', 'MAİL'],
      },
      { key: 'CEP / TEL', label: 'Phone', required: false, alternatives: ['phone', 'TELEFON', 'GSM', 'MOBILE'] },
      { key: 'TELEGRAM', label: 'Telegram Chat ID', required: false, alternatives: ['TELEGRAM CHAT ID', 'telegram_chat_id', 'TELEGRAM ID'] },
      {
        key: 'PREFERRED CHANNELS',
        label: 'Preferred Channels',
        required: false,
        alternatives: ['TERCİH EDİLEN KANALLAR', 'preferred_channels', 'KANALLAR', 'CHANNELS'],
      },
      { key: 'NOT', label: 'Notes', required: false, alternatives: ['not', 'NOTLAR', 'Notes', 'AÇIKLAMA'] },
      { key: 'USE CUSTOM MARGIN', label: null, required: false, alternatives: ['use_custom_margin', 'CUSTOM MARGIN', 'ÖZEL KAR'] },
      { key: 'MARGIN RATE (%)', label: null, required: false, alternatives: ['margin_rate', 'MARGIN RATE', 'KAR ORANI', 'KAR %'] },
    ],
    example: {
      FİRMA: 'ABC Lojistik',
      'MENŞEİ': 'İstanbul',
