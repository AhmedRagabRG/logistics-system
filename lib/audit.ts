...
export async function logPricingEvent(event: PricingEvent): Promise<void> {
  await pool.execute(
    ...
    [
      event.event_type,
      event.admin_id ?? null,
      event.details ? JSON.stringify({ quote_id: event.quote_id, ...event.details }) : null,
    ]
  );
}
...
export async function logVendorEvent(event: VendorEvent): Promise<void> {
  await pool.execute(
    ...
    [
      event.event_type,
      event.details ? JSON.stringify({ quote_id: event.quote_id, vendor_id: event.vendor_id, ...event.details }) : null,
    ]
  );
}
